...
const VerificationResetPassword: FC<Props> = ({ setRoute }) => {
  const { token } = useSelector((state: any) => state.auth)
  const [activeResetPassword, { isSuccess, error }] =
    useActiveResetPasswordMutation()
  const [invalidError, setInvalidError] = useState<boolean>(false)

  useEffect(() => {
    if (isSuccess) {
      toast.success('Active reset password successfully!')
